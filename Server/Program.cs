...
class Program
{
    ...
    private static async Task HandleClientAsync(TcpClient client)
    {
        ...
        using (var writer = new StreamWriter(stream) { AutoFlush = true })
        {
            ...
            while ((line = await reader.ReadLineAsync()) != null)
            {
                ...
                {
                    case "REQUEST":
                        await RequestPartner(userName, message);
                    ...
                    case "DISCONNECTFROMPART":
...

                        {
                            ...
                            clients[partnerUserName].PartnerUserName = null; // Установить партнёра в null
                            clients[userName].PartnerUserName = null;

                        }
                        break;
                    case "REJECT":
                        clients[message].Writer.WriteLineAsync($"Пользователь отказался");
                        break;
                    default:
    ...
    private static async Task RequestPartner(string userName, string partnerUserName)
    {
    ...
    private static async Task SelectPartnerAsync(string userName, string partnerUserName)
    {
        ...
        if (clients.ContainsKey(partnerUserName))
        {
            ...
            {
                await clients[userName].Writer.WriteLineAsync("ERROR: Вы уже подключены к другому клиенту.");
                return;
            }
    ...
    private static async Task SendConnectedClientsListAsync()

    {

        var connectedClients = string.Join(", ", clients.Keys);
...

        {
            ...
            await client.Value.Writer.WriteLineAsync($"CONNECTED {connectedClients}");
